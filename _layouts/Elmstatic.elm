...
decodePost =
    Json.succeed Post
        |> JsonPipeline.custom decodeContent
        |> JsonPipeline.required "date" Json.string
        |> JsonPipeline.custom decodeFormat
        |> JsonPipeline.required "link" Json.string
        |> JsonPipeline.required "section" Json.string
        |> JsonPipeline.required "siteTitle" Json.string
        |> JsonPipeline.required "tags" (Json.list Json.string)
        |> JsonPipeline.required "title" Json.string
        |> JsonPipeline.required "thumb" Json.string
...
htmlTemplate title contentNodes =
    node "html"
        ...
        [ node "head"
           ...
           -- , node "meta" [ attribute "name" "viewport", attribute "content" "width=device-width, initial-scale=1.0"] []
